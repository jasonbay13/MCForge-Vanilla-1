/*
 *About 50% of this works goes to sircmpwn 
 */
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using MCForge.Network.Packets;
...
namespace MCForge.Network {
    public abstract class Packet {
        ...
        private static Type[] Packets = new Type[] {
            ...
            typeof(PacketPing), // 0x01
            typeof(PacketWorldStart), // 0x02
            typeof(PacketWorldChuck), // 0x03
            typeof(PacketWorldStop), // 0x04
            typeof(PacketPlayerSetBlock), // 0x05
            typeof(PacketServerSetBlock), // 0x06
            typeof(PacketSpawn), // 0x07
            typeof(PacketSetPosition), // 0x08
            typeof(PacketInvalid), // 0x09
            typeof(PacketInvalid), // 0x0a
            typeof(PacketInvalid), // 0x0b
            typeof(PacketDespawn), // 0x0c
            typeof(PacketChat), // 0x0d
            typeof(PacketDisconnect), // 0x0e
        ...
        public abstract void ReadPacket(Player p);
        public abstract void WritePacket(Player p);
        ...
        public static string ReadString(Stream mStream) {
            byte[] bytes = new byte[64];
            mStream.Read(bytes, 0, 64);
            return ReadString(bytes);
        }
        public static int ReadInt(Stream mStream) {
            byte[] bytes = new byte[4];
            mStream.Read(bytes, 0, 4);
            return ReadInt(bytes);
        }
        public static double ReadDouble(Stream mStream) {
            byte[] bytes = new byte[8];
            mStream.Read(bytes, 0, 8);
            return ReadDouble(bytes);
        }
        public static long ReadLong(Stream mStream) {
            byte[] bytes = new byte[8];
            mStream.Read(bytes, 0, 8);
            return ReadLong(bytes);
        }
        public static short ReadShort(Stream mStream) {
            byte[] bytes = new byte[2];
            mStream.Read(bytes, 0, 2);
