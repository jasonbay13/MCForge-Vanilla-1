    ...
    public partial class Player {
            ...
            try {
                ...
                switch (msg) {
                    case 0:
                        ...
                        break; // login
                    case 2:
                        SMPKick("This is not an SMP Server!");
                        break; // SMP Handshake packet
                    case 5:
                        length = 8;
                        break; // blockchange
                    case 8:
        ...
        protected void HandleLogin(byte[] message) {
            try {
                ...
                bool banned = false;
                foreach (string line in File.ReadAllLines("Bans/BanInfo.txt")) {
                    if (Username == line.Split('`')[0]) {
                        BanReason = line.Split('`')[1];
                    }
                }
                ...
                foreach (string line in File.ReadAllLines("Bans/UsernameBans.txt")) {
                    if (line == Username)
                        banned = true;
                }
                if (banned) {
                    if (BanReason != "No reason specified.") {
                        SKick("You are banned because " + BanReason);
                    }
                    else {
                        SKick("You are banned!");
                    }
                }
                string verify = enc.GetString(message, 65, 32).Trim();
                byte type = message[129];
                if (!VerifyAccount(Username, verify))
                    return;
                if (Server.Verifying)
                    verified = false;
                else
                    verified = true;
                if (version != ServerSettings.Version) {
                    SKick("Wrong Version!");
                    return;
                }
                try {
        ...
        protected void HandleChat(byte[] message) {
            ...
            if (Server.moderation && !voiced && !Server.devs.Contains(Username)) {
        ...
        protected void SendPacket(packet pa) {
            ...
            for (int i = 0; i < 3; i++) {
                try {
                    Socket.BeginSend(pa.bytes, 0, pa.bytes.Length, SocketFlags.None, delegate(IAsyncResult result) {
                    }, null);
                    return;
                }
                catch {
        ...
        public void SendBlockChange(ushort x, ushort z, ushort y, byte type) {
            if (x < 0 || y < 0 || z < 0 || x >= level.Size.x || y >= level.Size.y || z >= level.Size.z)
                return;
